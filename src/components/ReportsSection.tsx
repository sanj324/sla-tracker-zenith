...
const ReportsSection = ({ banks }: ReportsSectionProps) => {
  ...
  const generateReport = (status: "pending" | "completed") => {
    ...
    const headers = ["Sr No,Bank Name,Branches,Send Mail,Courier Date,Received in TM,In Franking,Last Agreement Date,New Agreement Date,Old Amount,New Amount,Resend"];
    const csvData = filteredBanks.map((bank, index) => (
      `${index + 1},${bank.name},${bank.branches || ''},${bank.mailStatus || ''},${bank.courierDate || ''},${bank.receivedInTM ? 'Yes' : 'No'},${bank.inFranking ? 'Yes' : 'No'},${bank.lastAgreementDate || ''},${bank.newAgreementDate || ''},${bank.oldAmount || ''},${bank.newAmount || ''},${bank.resend ? 'Yes' : 'No'}`
    ));
